...
public class Solution {
  public double AngleClock(int hour, int minutes) {
    double hour2 = ((hour%12) / 12.0) * 60 + (minutes / 60.0)*5;

    hour2 = (hour2 / 60.0) * 360.0;
    double minutes2 = (minutes / 60.0) * 360.0;

    double a = Math.Abs(minutes2 - hour2);
    double b = 360.0f - a;
    double answer = Math.Min(a, b);
...
public class MainClass {
  ...
  public static void Main(string[] args) {
    ...
    foreach (var tc in testcases) {
      var result = solution.AngleClock(tc.a, tc.b);
