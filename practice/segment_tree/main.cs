...
namespace Practice {
    using Vertex = (int value, bool marked);
    ...
    public class SegmentTreeMaxCount : SegmentTree<(int max, int count)> {
        public SegmentTreeMaxCount(int[] nums) : base(nums) { }
        ...
        public override (int max, int count) Construct(int value) { return (value, 1); }
    ...
    public class ZeroCount : SegmentTree<int> {
        ...
        public override int Construct(int value) {
            return (value == 0 ? 1 : 0);
        }
    ...
    public class MainClass {
        ...
        public static void Main(String[] args) {
            ...
            for (int N = 10; N < 50; N++) {
                ...
                SegmentTreeSum tree = new SegmentTreeSum(nums);

                for (int j = 0; j < 10; j++) {
                    int pos = Random.Shared.Next(0, N);
                    int val = Random.Shared.Next(-100, 100);
                    tree.Update(pos, val);
                    nums[pos] = val;

                    for (int left = 0; left < N; left++) {
                        for (int right = left + 1; right < N; right++) {
                            var got = tree.Search(left, right);
